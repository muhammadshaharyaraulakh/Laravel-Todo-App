    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex items-center space-x-8">
                <x-nav-link :href="auth()->check() ? route('dashboard') : route('login')" :active="request()->routeIs('dashboard')">
                    {{ __('Home') }}
                </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @else
                ...
                <x-responsive-nav-link :href="route('tasks.create')">{{ __('Create Task') }}</x-responsive-nav-link>
                <x-responsive-nav-link :href="route('tasks.history')">{{ __('History') }}</x-responsive-nav-link>
            @endguest
        </div>
